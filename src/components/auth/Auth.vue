<template>
  <div class="auth">
    <div class="auth__wrapper">
      ...
      <h3 class="auth__header">Log in</h3>
      <p class="auth__header-text">Welcome back!</p>
      <div class="auth__block">
        <p class="auth__form-text">Email address</p>
      ...
      <div class="auth__block">
        <div class="auth__block-container">
          <p class="auth__form-text">Password</p>
          <a href="" class="auth__form-link">Forgot your password?</a>
        </div>
        <form class="auth__form">
          <input
      ...
      <div class="auth__block">
        <a class="auth__link" @click="auth">Log in</a>
        <p v-show="errMessage" class="auth__error">{{ errMessage }}</p>
      </div>
      <div class="auth__bottom">
        <p class="bottom-text">Don't have an account yet?</p>
        <a class="auth__reg-link" @click="$emit('change', 'reg')">Join us</a>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      loginEmailURL: 'https://pepperfm.ru/api/v1/login-email',
      ...
      data: {
        ...
        grant_type: 'password',
        client_id: '2',
        client_secret: 'BSLYz8iMrooeTzSLcfeHafGAq38721l1PlxrirfZ',
      },
      errMessage: '',
    };
  },
  methods: {
    ...
    async auth() {
      const res = await this.$http
        .post(this.loginEmailURL, this.data)
        .then((data) => {
          if (data.status === 200) {
            this.$store.commit('saveToken', data.data.access_token);
            // save to store
